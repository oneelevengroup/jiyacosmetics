...
export default function Footer({ locale = "en" }: { locale?: Locale }) {
  const { business, nav, serviceCategories } = getSite(locale);
      ...
      <div className="container-site py-20 lg:py-28">
        <div className="grid gap-14 lg:grid-cols-12">
          ...
          <div className="lg:col-span-4">
            <Logo size="footer" locale={locale} />
          ...
          <div className="lg:col-span-2">
            <h3 className="eyebrow">{ui.explore}</h3>
          ...
          <div className="lg:col-span-3">
            <h3 className="eyebrow">{ui.servicesLabel}</h3>
            <ul className="mt-6 space-y-3">
          ...
          {/* Contact */}
          <div className="lg:col-span-3">
            <h3 className="eyebrow">{ui.visit}</h3>
        ...
        <div className="mt-16 flex flex-col items-start justify-between gap-4 border-t border-cream/10 pt-8 sm:flex-row sm:items-center">
          <p className="font-sans text-xs tracking-wide text-cream/45">
            © {new Date().getFullYear()} {business.fullName}. {ui.allRightsReserved}.
            <span className="mx-2 text-cream/25">·</span>
            <Link href={localePath(locale, "/privacy")} className="link-underline hover:text-cream/70">
              {ui.privacyPolicy}
            </Link>
          </p>
